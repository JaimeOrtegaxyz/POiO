/* POiO companion — ENCODER-ONLY friction probe (the control).
   The opposite bet to voice: put the LLM's intelligence into PREDICTION, not
   listening. The device already knows the recipe, so it knows what you probably
   used. At plate-up it shows a pre-computed "used tonight?" checklist — already
   ticked — and one press confirms the lot. N one-by-one edits collapse to 1
   confirm + the occasional un-tick. If this feels fine, the mic earns nothing:
   that's the whole point of a control. Adds ZERO hardware over PROTOTYPE.md.
   Same e-paper physics, same cooking flow, same confirm screen as the others. */
(() => {
  'use strict';
  ...
  // The LLM's prediction of what the recipe consumed — computed WHILE you cook,
  // so it's already waiting at plate-up. No mic, no round-trip to feel.
  const CONSUMED = [
    { nm: 'tostadas', to: 'out' },
  ...
  const state = {
    ...
    timerId: null, timeLeft: 0, appliedN: 0,
  };
  const MENU = [
  ...
  const VIEW = {
    today() {
      ...
      return `
        ...
        <div class="talk-hint"><span class="pip"></span>turn to choose · <b>press</b> to open</div>`;
    },
    ...
    review() {
      ...
      return `
        ...
        <div class="diff-src">predicted from the tinga · computed while you cooked</div>
    ...
    saved() {
      return `<div class="applied"><div class="chk">✓</div><div class="big">Pantry updated</div>
        <div class="sub">${state.appliedN} change${state.appliedN === 1 ? '' : 's'} · one press · shopping caught up</div></div>`;
    },
    pantry() {
      ...
      return `
        ...
        <div class="talk-hint" style="margin-top:10px"><span class="pip"></span><b>press</b> ● cycle status — one at a time</div>`;
    },
    shopping() {
      let html = '', last = null;
  ...
  // the control's move: the checklist is already computed — no record, no wait.
  // cursor lands on "confirm" so the whole thing is a single press if it's right.
  function openReview() { state.diff = CONSUMED.map((d) => ({ ...d, drop: false })); state.dHl = state.diff.length; setView('review'); }
  ...
  function back() { if (state.view === 'review') { setView('pantry'); return; } if (state.view !== 'today') home(); }
