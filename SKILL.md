...
The user's equipment lives in `references/equipment.md` (personal, gitignored — generated by the setup interview). Read it and let it shape suggestions: which methods to reach for, which to skip, which to save for weekends. If the file is missing, assume stovetop + oven only and let the interview fill it in.

## How to Start

**Step 0 — Bootstrap check.** Before anything else, verify that `pantry.md`, `references/regional-context.md`, and `references/equipment.md` exist in the skill directory. If any is missing, do NOT proceed to suggestions — open `setup/interview.md` and run it as a script for the missing file(s). The interview generates the personal files; once it's done, continue with Step 1. The committed `*.example.md` files are structural references for the interview, not files to read directly.
...
**Step 3:** Read `references/style-guide.md` for flavor profile families, sauce archetypes, and assembly patterns, and `references/equipment.md` for what this kitchen cooks on.
